...
const codeconMetadataUrl =
  "https://ipfs.io/ipfs/QmdMbGwGLmC5iNmv1hzRvJQpzrKmKfHjbB34k4e8AfqKay?filename=Codecon%20Tech%20Event";
const webArMetadataUrl =
  "https://ipfs.io/ipfs/QmSzFfx3rNqdJwSsrFpfMcxZCncaATsCceaFEr6Lmq3VUz?filename=Showing%20off%20WebAR";
...
async function setupMarket(marketplaceAddress, nftAddress) {
  ...
  const codeconMarketTx = await marketplaceContract.createMarketItem(nftContractAddress, codeconTokenId, price, {
    value: listingFee,
  });
  ...
  console.log(
    `${acc1.address} minted tokens ${dogsTokenId}, ${techEventTokenId}, ${codeconTokenId} and ${webArTokenId} and listed them as market items`,
  );
  ...
  await marketplaceContract
    .connect(acc2)
    .createMarketItem(nftContractAddress, yellowTokenId, price, { value: listingFee });
  await marketplaceContract
    .connect(acc2)
    .createMarketItem(nftContractAddress, ashleyTokenId, price, { value: listingFee });
  ...
  await marketplaceContract
    .connect(acc2)
    .createMarketItem(nftContractAddress, dogsTokenId, price, { value: listingFee });
